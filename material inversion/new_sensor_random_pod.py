...
# resolution-independent, using the same network with pod_main.py
# new sensor pod-based inversion, 400 random sensors
import numpy as np
...
if __name__ == '__main__':
    ...
    # === model setting ===
    model = WGanGP3(coords_sensor_new_T, Opt.resp_features, means_nn, components_nn)
    g_iter = 20000
    ...
    for i in tqdm.tqdm(range(opt_iter)):
        ...
        loss.backward()
        map_optimizer.step()
        iter_array[i, :6] = z_T.data.cpu().numpy()
    ...
    plot_axes_scatter(axes[1][1], coords_sensor_new[:, 0], coords_sensor_new[:, 1], obs_disp[:n_points], "observation_x")
    plot_axes_pcolor(axes[1][2], x_mesh_sensor, y_mesh_sensor, posterior_resp_mean[625:], "posterior_mean_disp_y")
    plot_axes_scatter(axes[1][3], coords_sensor_new[:, 0], coords_sensor_new[:, 1], obs_disp[n_points:], "observation_y")
