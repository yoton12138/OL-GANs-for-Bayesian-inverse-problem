...
if __name__ == '__main__':
    ...
    # === model setting ===
    model = WGanGP4(coords_sensor_T, Opt.resp_features)
    g_iter = 20000
    # model.load_models(f"save_models/exp18/state_{g_iter}.pt")
    ...
    init_position = z_map  # 使用最大似然估计获得初始点
    burn = 5000
    ...
    print(f"弹性模量场的相对l2误差： {error_l2_e}, 在 3 sigma区间的比例： {round(in3std_ration,2)*100} %")
    print(f"x  位移场的相对l2误差： {error_l2_x}")
    print(f"y  位移场的相对l2误差： {error_l2_y}")
