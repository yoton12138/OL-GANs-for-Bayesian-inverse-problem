...
if __name__ == '__main__':
    ...
    # === model setting ===
    model = WGanGP(coords_params_T, coords_sensor_T, Opt.resp_features)
    g_iter = 20000
    ...
    axes[0].fill_between(x_lin, lower.flatten(), upper.flatten(), alpha=0.5, rasterized=True, label="Epistemic uncertainty")
    axes[0].legend()
    ...
    error_l2_resp = np.linalg.norm(posterior_resp_mean - ref_resp_sensor.squeeze(), 2) / np.linalg.norm(ref_resp_sensor.squeeze(), 2)
