...
class DeepONet(nn.Module):
    def __init__(self, noise_feature, coords_features, p, share=1):
        ...
        self.bias_params = torch.zeros(1, requires_grad=True)
...
class PodDeepONet(nn.Module):
    def __init__(self, noise_feature, p, trunk_means, trunk_components):
        ...
        self.branch = BranchNet(noise_feature, p, width=32)  # 默认32
...
class WGanGP(object):
    def __init__(self, coords_params_T, coords_T, resp_features, cuda=True):
        self.coords_T = coords_T
        self.coords_params_T = coords_params_T
        self.resp_features = resp_features

        self.gen_params = DeepONet(Opt.noise_features, 1, Opt.p)
        ...
        for g_iter in range(self.start_g_iter + 1, self.generator_iters + 1):
            ...
            for batch_iter in range(batch_num):
                ...
                for d_iter in range(self.critic_iter):
                    ...
                    fake_data_batch = self.generate_fake_data(z, self.coords_params_T, self.coords_T)
    ...
    def generate_fake_data(self, z, coords_params_T, coords_T):
        fake_data_params = self.gen_params(z, coords_params_T).reshape(-1, Opt.params_features)
        fake_data_resp = self.gen_resp(z, coords_T).reshape(-1, int(Opt.resp_features))
    ...
    def save_images(fake_data_batch, g_iter):
        ...
        xx, yy = Opt.x_mesh_sensor, Opt.t_mesh_sensor  # 中途加入的变量
        ...
        ax0 = axes[0][0].plot(samples_params[0])
        axes[0][0].set_title("params")
        ax1 = axes[0][1].plot(samples_params[1])
        axes[0][1].set_title("params")
        ax2 = axes[0][2].plot(samples_params[2])
        axes[0][2].set_title("params")

        ax3 = axes[1][0].contourf(xx, yy, samples_resp[0].reshape(resolution, resolution))
        plt.colorbar(ax3, ax=axes[1][0])
        ax4 = axes[1][1].contourf(xx, yy, samples_resp[1].reshape(resolution, resolution))
        plt.colorbar(ax4, ax=axes[1][1])
        ax5 = axes[1][2].contourf(xx, yy, samples_resp[2].reshape(resolution, resolution))
...
class WGanGP3(WGanGP):
    def __init__(self, coords_params_T, coords_T, resp_features, params_means_nn, params_components_nn,
                 resp_means_nn, resp_components_nn, cuda=True):
        self.coords_T = coords_T
        self.coords_params_T = coords_params_T
        self.resp_features = resp_features

        self.gen_params = PodDeepONet(Opt.noise_features, Opt.p, params_means_nn, params_components_nn)
        self.gen_resp = PodDeepONet(Opt.noise_features, Opt.p, resp_means_nn, resp_components_nn)
        self.disc = Fnn(Opt.layers_size_disc, sn=False)
    ...
    def generate_fake_data(self, z, coords_params_T, coords_T):
        fake_data_params = self.gen_params(z, coords_params_T).reshape(-1, Opt.params_features)
        fake_data_resp = self.gen_resp(z, coords_T).reshape(-1, Opt.resp_features)
        fake_data_batch = torch.cat([fake_data_params, fake_data_resp], dim=1)
