...
def plot_modes(xx, yy, components, title):
    ...
    def plot_pcolor(ax, xx, yy, zz, str,):
        ax0 = ax.pcolor(xx, yy, zz)
        ax.set_title(str)
        plt.colorbar(ax0, ax=ax)

    res = 9
    plot_pcolor(axes[0][0], xx, yy, components[:, 0].reshape(res, res), "first resp mode")
    plot_pcolor(axes[0][1], xx, yy, components[:, 1].reshape(res, res), "second resp mode")
    plot_pcolor(axes[1][0], xx, yy, components[:, 2].reshape(res, res), "third resp mode")
    plot_pcolor(axes[1][1], xx, yy, components[:, 3].reshape(res, res), "fourth resp mode")
